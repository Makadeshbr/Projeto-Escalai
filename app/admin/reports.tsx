...
import { Target, Search, CheckCircle2, Package, Cloud, UserX, UserCheck, ShieldAlert, BadgeInfo } from 'lucide-react-native';
...
export default function AdminReportsScreen() {
    ...
    const fetchGlobalReports = useCallback(async (showRefreshIndicator = false) => {
        ...
        try {
            ...
            // Indexando Rotas Finalizadas
            const assignments = rawAssignments.filter(a => {
                const p = a._payload || a;
                return p.status === 'completed' || p.status === 'confirmed';
            });
    ...
    const renderDriverCard = (driver: DriverReport, idx: number) => {
        const initials = driver.driverName.substring(0, 2).toUpperCase();

        return (
            <View key={driver.id} className="bg-surface rounded-2xl border border-border p-4 mb-4 shadow-sm relative overflow-hidden">
                ...
                <View className="flex-row items-center mb-4 mt-1">
                    ...
                    <View className="w-14 h-14 rounded-full border-2 border-primary/30 items-center justify-center bg-background overflow-hidden mr-4">
                        {driver.profilePicUrl ? (
                            <Image source={{ uri: driver.profilePicUrl }} className="w-full h-full" resizeMode="cover" />
                        ) : (
                ...
                <View className="flex-row gap-2">
                    <View className="flex-[1.2] bg-[#1a1d2e] border border-border rounded-xl p-3 items-center justify-center">
                        ...
                        <Text className="text-2xl font-spaceGroteskBold text-white">{driver.metrics.routes}</Text>
                    ...
                    <View className="flex-1 bg-orange-500/5 border border-orange-500/10 rounded-xl p-3 items-center justify-center">
                        ...
                        <Text className="text-2xl font-spaceGroteskBold text-orange-500">{driver.metrics.sacas}</Text>
                    ...
                    <View className="flex-1 bg-background border border-border rounded-xl p-3 items-center justify-center">
                        ...
                        <Text className="text-2xl font-spaceGroteskBold text-[#94a3b8]">{driver.metrics.offDays}</Text>
                ...
                {driver.metrics.sdd > 0 && (
                    <View className="mt-3 flex-row items-center justify-center gap-1 opacity-50">
                        <Text className="text-primary text-[10px] uppercase font-spaceGroteskBold tracking-wider">* Fez {driver.metrics.sdd} Rota{driver.metrics.sdd > 1 ? 's' : ''} SDD (Same Day)</Text>
                    </View>
                )}
            </View>
        );
                ...
                {isLoading ? (
                    ...
                    >
                        {filteredReports.length === 0 ? (
                            ...
                            filteredReports.map((driver, index) => renderDriverCard(driver, index))
                        )}
                    </ScrollView>
                )}
            </View>
